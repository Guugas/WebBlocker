...
Event.add(document, "DOMContentLoaded", function() {
  ...
  Event.add("ck_porn", "click", () => {
    var checkBox = document.getElementById("ck_porn");

    if (checkBox.checked) {
      pornSites.map(porn => {
        if (!list_bs.includes(porn)) list_bs.push(porn);
      });
    } else {
      list_bs = list_bs.filter((value, index, arr) => {
        if (!pornSites.includes(value)) return value;
      });
    }
  });

  Event.add("ck-social-media", "click", () => {
    var checkBox = document.getElementById("ck-social-media");

    if (checkBox.checked) {
      socialMediasSites.map(socialMedia => {
        if (!list_bs.includes(socialMedia)) list_bs.push(socialMedia);
      });
    } else {
      list_bs = list_bs.filter((value, index, arr) => {
        if (!socialMediasSites.includes(value)) return value;
      });
    }
  });
...
var list_bs = (list_bw = list_ts = list_pf = []),
  password = null;
...
var save_options = function() {
  var content_filter = {
    ...
    advanced: {
      warning:
        Dom.get("adv_warning").value ||
        "Esta página não está disponível devido a restrições de administrador.",
  ...
  if (subscriptions.url.length > 0) {
    if (subscriptions.url != bg.prefs.subscriptions.url) {
      ...
      if (response) {
        subscriptions.content_filter.block.sites = response.content_filter.block
          .sites
          ? response.content_filter.block.sites
          : [];
        subscriptions.content_filter.block.words = response.content_filter.block
          .words
          ? response.content_filter.block.words
          : [];
        subscriptions.content_filter.trust.sites = response.content_filter.trust
          .sites
          ? response.content_filter.trust.sites
          : [];
        subscriptions.profanity_filter.words = response.profanity_filter.words
          ? response.profanity_filter.words
          : [];
      ...
      subscriptions.content_filter.block.sites =
        bg.prefs.subscriptions.content_filter.block.sites;
      subscriptions.content_filter.block.words =
        bg.prefs.subscriptions.content_filter.block.words;
      subscriptions.content_filter.trust.sites =
        bg.prefs.subscriptions.content_filter.trust.sites;
      subscriptions.profanity_filter.words =
        bg.prefs.subscriptions.profanity_filter.words;
      subscriptions.last_update = bg.prefs.subscriptions.last_update
        ? bg.prefs.subscriptions.last_update
        : new Date().getTime();
    }
  } else {
    subscriptions.enabled = false;
